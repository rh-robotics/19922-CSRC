 ...
 */
@Autonomous (name = "Odo Left")
public class OdometryAutonomousLeft extends OpMode {
    HWC robot;
    ...
    public void loop() {
        if(robot.cv() == 1){
            robot.odoStrafeLeft(strafeDist);
            ...
            robot.odoStrafeRight(strafeShort);
            robot.odoDrive(tapeDistSide);
            robot.sleepDeliver(deliveryTime);}
            ...
            if (robot.cv() != 0){
                ...
                robot.odoStrafeLeft(2*strafeDist);
                robot.odoDrive(tapeDistSide);
                robot.sleepDeliver(deliveryTime);
            }
        }
